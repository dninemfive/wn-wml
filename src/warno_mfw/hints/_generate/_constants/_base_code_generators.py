...
from warno_mfw.hints._generate._types._member_def import MemberDef
...
def _resolve_sig_generator(name: str) -> str:
    return f'_resolve_{name}(s: str) -> str:\n\t'
...
def _format_resolver_generator(name: str, prefix: str, require_quotes: bool) -> str:
    return f'{_resolve_sig_generator(name)}return {_format_resolver.__name__}(s, {prefix}, {require_quotes})'